...
**外部工具（MCP，二次开发新增）**
- 设置 → 工具（MCP）：添加本机 stdio 或 Streamable HTTP 服务，发现并勾选工具，按群聊／私聊限定范围，并在页面试调用。
- 新服务默认不启用工具；连接和调用有超时、参数校验与结果长度控制。使用步骤、配置示例及当前限制见 [MCP 工具说明](docs/mcp-tools.md)。
...
npm test             # 全部回归：功能、界面、工具、记忆、权限与响应规则
```

### 群聊响应

群聊默认根据热聊、冷场和累计消息保底规则自动参与：热聊统计最近一分钟，保底默认累计 20 条未触发消息，每群滚动一分钟最多自动参与两次；私聊沿用原响应档位。聊天设置可单独调整每个群聊或私聊，完整条件和额度说明见 [响应规则](docs/response-rules.md)。
...
本分支已移除原作者的意见收集、金句上传、网站与群入口，以及自动用量遥测和作者版本检查。聊天存档、记忆和用量统计仍保存在本地。

模型、搜索、MCP 和 QQ 协议端会按配置正常联网；远程价格表仅在用户配置 URL 时启用。审查范围和移除项见 [联网审查记录](docs/privacy-audit.md)。
